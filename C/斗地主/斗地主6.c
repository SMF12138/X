...
char* Dapai(int people,int paixing,char pai,char Pai[],char X[],int i,char*Temp,char*anoTemp){
	int count = 0;
	if(i==17){
		switch(people){
		case 1://玩家是农民 
...
char* Caice(int people,char Pai[],char X[],int i,char*Temp,char*anoTemp){
	...
	switch(paixing){
		case 1:
			printf("打出的是单几\n");
			scanf(" %c",&pai);
			return Dapai(people,paixing,pai,Pai,X,i,Temp,anoTemp);
		case 2:		
			printf("打出的是对几\n");
			scanf(" %c",&pai);
			return Dapai(people,paixing,pai,Pai,X,i,Temp,anoTemp);
		case 3:
			...
			switch(pai){
				case '3':temp = Dapai(people,paixing,pai,Pai,X,i,Temp,anoTemp);	
				if(paixing==4)	temp = Dapai(people,paixing,pai,Pai,temp,i,Temp,anoTemp);
								p++;
				case '4':if(pai=='4'){
							temp = Dapai(people,paixing,pai,Pai,X,i,Temp,anoTemp);
							if(paixing==4)	temp = Dapai(people,paixing,pai,Pai,temp,i,Temp,anoTemp);
						}
						else {
							pai='4';
							temp = Dapai(people,paixing,pai,Pai,temp,i,Temp,anoTemp);
							if(paixing==4)	temp = Dapai(people,paixing,pai,Pai,temp,i,Temp,anoTemp);
						}
								p++;
				case '5':if(pai=='5'){
		...
		case 5:		
			...
			scanf(" %c",&pai);
			temp = Dapai(people,paixing,pai,Pai,X,i,Temp,anoTemp);
			printf("带（1.单 2.对 0.不带）\n");
			scanf("%d",&paixing);
			if(paixing==1) printf("单几？\n");
			else  printf("对几？\n");
			scanf(" %c",&pai);
			return Dapai(people,paixing,pai,Pai,temp,i,Temp,anoTemp);
		case 6:		
			printf("打出的是三几\n");
			scanf(" %c",&pai);
			temp = Dapai(people,paixing,pai,Pai,X,i,Temp,anoTemp);
...
int main() {
	char* Temp = (char*)calloc(54, sizeof(char));
	char* anoTemp = (char*)calloc(54, sizeof(char));
	...
	int c;
	while ((c = getchar()) != '\n' ) { }
	char* MyPai = Paixu(You);//返回的是排序好的You 
	printf("您摸到的牌是\n");
	BianliPai(MyPai);
	...
	X=Caice(people,Pai,X,i,Temp,anoTemp);
	key = 0;
	for(j=0;j<54;j++){
		key = *(X+j)+key;
	}
	...
	free(SP);free(anoTemp);
	free(MyPai);free(Temp);
	printf("游戏结束\n");
	return 0;
}
