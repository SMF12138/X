...
public class Tetris extends JPanel {
    ...
    //声明游戏的三种状态
    public static final int PLATING = 0;
    public static final int PAUSE = 1;
    public static final int GAMEOVER = 2;
    //声明当前游戏状态值
    private int game_state;
    //声明显示游戏状态的数组
    String[] show_state = {"P[暂停]", "C[继续]", "S[重新开始]"};
    ...
    static {
        try {
            ...
            backImage = ImageIO.read(new File("src\\images\\backImage.png"));
        } catch (IOException e) {
    ...
    private void paintState(Graphics g) {
        if (game_state == PLATING) {
            g.drawString(show_state[0], 500, 620);
        } else if (game_state == PAUSE) {
            g.drawString(show_state[1], 500, 620);
        } else if (game_state == GAMEOVER) {
            g.drawString(show_state[2], 500, 620);
            g.setColor(Color.cyan);
            g.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 60));
            g.drawString("游戏结束!", 30, 400);
        }
    }
    ...
    private void paintScore(Graphics g) {
        ...
        g.drawString("行数:" + totalLine, 500, 420);
    }
